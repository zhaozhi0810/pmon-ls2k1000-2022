...
synopGMACdevice *g_gmacdev_0;
synopGMACdevice *g_gmacdev_1;
...
#endif
static void zx5201_config_init(synopGMACdevice *gmacdev)
{
    u16 data;
    
    //data = 0x6400;
    //synopGMAC_write_phy_reg((u64 *)gmacdev->MacBase, gmacdev->PhyBase, 0x12, data);  

    //synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x04, &data);
    //data |= 0x0800;
    //synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x04, data);

    //synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x09, &data);
    //data |= 0x0300;
    //synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x09, data);
    
    synopGMAC_read_phy_reg(gmacdev->MacBase,gmacdev->PhyBase,0x00,&data);
    data |= 0x8000;
    synopGMAC_write_phy_reg(gmacdev->MacBase,gmacdev->PhyBase,0x00,data);
}
...
#endif
{
    ...
    //printf("gmacdev:0x%x,MacBase:0x%x,PhyBase:0x%x\n",(int)gmacdev,gmacdev->MacBase,gmacdev->PhyBase);
    
    if (g_gmacdev_0 == NULL)
        g_gmacdev_0 = gmacdev;
    else if ((g_gmacdev_0 != gmacdev) && (g_gmacdev_1 == NULL))
        g_gmacdev_1 = gmacdev;
    ...
    else if ((data2 == 0x84b9) && ((data3 & 0x5000) == 0x5000))
    {
        printf("find phy ZX5201 \n");
        zx5201_config_init(gmacdev);
    }
    else
    {
        printf("==== Warning: unrecoganized gmac phy!\n reset phy\n");
        synopGMAC_read_phy_reg(gmacdev->MacBase,gmacdev->PhyBase,0x00,&data2);
        data2 |= 0x8000;
        synopGMAC_write_phy_reg(gmacdev->MacBase,gmacdev->PhyBase,0x00,data2);
    }
    return 0;
#else
    retval = rtl8211_config_init(gmacdev);
...
void cmd_getphy(int argc, char **argv)
{
    u16 data;
    u16 reg;
    int index;
    synopGMACdevice *g_gmacdev = NULL;
    if (argc < 2)
        return -1;
    ...
    if (index == 0)
        g_gmacdev = g_gmacdev_0;
    else
        g_gmacdev = g_gmacdev_1;
        
    printf("g_gmacdev:0x%x,MacBase:0x%x,PhyBase:0x%x\n", (int)g_gmacdev, g_gmacdev->MacBase, g_gmacdev->PhyBase);
    
    synopGMAC_read_phy_reg(g_gmacdev->MacBase, g_gmacdev->PhyBase, reg, &data);
    
    printf("get phy%d reg:0x%x data:0x%x.\n", index, reg, data);
    return 0;
}

void cmd_setphy(int argc, char **argv)
{
    int err;
    u16 data;
    u16 reg;
    ...
    if (index == 0)
        err = synopGMAC_write_phy_reg(g_gmacdev_0->MacBase, g_gmacdev_0->PhyBase, reg, data);
    else
        err = synopGMAC_write_phy_reg(g_gmacdev_1->MacBase, g_gmacdev_1->PhyBase, reg, data);
...
void sk21101_setphy_reset(void)
{
	synopGMAC_write_phy_reg(g_gmacdev_0->MacBase, g_gmacdev_0->PhyBase, 0, 0x9140);
	synopGMAC_write_phy_reg(g_gmacdev_1->MacBase, g_gmacdev_1->PhyBase, 0, 0x9140);   
}


void cmd_dumpphy(int argc, char **argv)
{
    int i;
    u16 data;
    int index;
    ...
    else
    {
        for (i = 0; i <= 31; i++)
        {
            if ((i == 0) || (i == 4) || (i == 7) || (i == 9) || (i == 16) || (i == 18) ||
                    (i == 20) || (i == 22) || (i == 24) || (i == 25) || (i == 26) || (i == 27) || (i == 28))
            {
                synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, i, &data);
                printf("\t %d   0x%04x \n", i, data);
            }
...
{
    {"Misc"},
    {"getphy", "", NULL, "get the phy reg of LS7A syn0 and syn1", cmd_getphy, 1, 5, 0},
    {"dumpphy", "", NULL, "dump the phy reg of LS7A syn0 and syn1", cmd_dumpphy, 1, 5, 0},
    {"setphy", "", NULL, "set the phy reg of LS7A syn0 and syn1", cmd_setphy, 1, 5, 0},
    {0, 0}
};
